import i18n from "i18next";
import { initReactI18next } from "react-i18next";

i18n.use(initReactI18next).init({
  resources: {
    tr: {
      translation: {
        topBarLeftButtonGroup: "Dil, tema ve ses seçimi buton grubu",
        topBarRightButtonGroup: "Uygulama sayaç bilgisi buton grubu",
        regularNeedButtonGroup: "Düzenli ihtiyaç buton grubu",
        topBarTrButton: "Türkçe dilini seçme butonu",
        topBarEnButton: "İngilizce dilini seçme butonu",
        topBarThemeButton: "Tema açma kapama butonu",
        topBarSoundButton: "Sesli okuma butonu",
        topBarInfoButton: "Uygulama sayaç bilgisi butonu",
        regularNeedInfoButton: "Düzenli ihtiyaç bilgisi butonu",
        regularNeedInfoTitle: "Düzenli İhtiyaç Nedir?",
        regularNeedInfoContent: "Düzenli Kan İhtiyacı, tek seferlik değil; tedavi süreci boyunca belirli aralıklarla devam eden kan veya kan bileşeni ihtiyacını ifade eder.\nÖrneğin; onkoloji tedavisi gören hastalar, organ nakli sürecindeki bireyler veya bazı kronik hastalıklar nedeniyle düzenli transfüzyon alan kişiler bu kapsama girer.\nTek seferlik veya kısa süreli ihtiyaçlar için bu seçeneği işaretlemeyiniz.",
        greeting: "Geçmiş olsun dileklerimizi iletiyoruz. Umarız en kısa sürede uygun kan bulunur ve sağlığınıza kavuşursunuz. Yanınızdayız!",
        close: "Kapat",
        headerTitle: "Kan İlanı Oluşturma Asistanı",
        headerDesc: "Formu doldurduktan sonra “Son Adım” butonuna tıklayarak oluşturulan görseli bizimle ve çevrenizle paylaşabilirsiniz.",
        fillAllFields: "<strong>Uyarı:</strong> Lütfen formdaki <strong>{{missingFields}}</strong> {{noun}} eksiksiz doldurunuz.\nGirilen bilgiler, kan bağışı sürecinde doğru ve hızlı iletişim kurulabilmesi için gereklidir.",
        fillAllFieldsSingleNoun: "alanını",
        fillAllFieldsMultipleNoun: "alanlarını",
        requiredText: "Bu alan zorunludur.",
        downloadButtonAlt: "Son Adım: İlanı Galerine Kaydet",
        phoneNumberInvalid: "Lütfen geçerli bir telefon numarası giriniz.\nNumara başında “0” olacak şekilde 11 haneli olmalıdır.\n(örn. 05XX XXX XX XX)",
        bloodGroup: "Kan Grubu",
        bloodGroupType: "Kan Grubu Türü",
        bloodType: "Kan Türü",
        donationType: "Bağış Türü",
        fullName: "Hasta Adı",
        phone: "Telefon",
        date: "Tarih",
        regularNeedDate: "Düzenli İhtiyaç",
        hospitalName: "Hastane Adı",
        location: "Kan Bağışının Yapılabileceği Merkezler",
        select: "Seçiniz",
        submit: "Gönder",
        legalNotice: "Tarafınızca girilen kişisel veriler, herhangi bir fiziki veya dijital ortamda kaydedilmez, depolanmaz ya da üçüncü kişilerle paylaşılmaz. Bu veriler yalnızca “Kan İlanı Formu” üzerinde anlık olarak görüntülenir ve görüntüleme sonrasında tamamen ortadan kaldırılır.",
        redBlood: "Kırmızı Kan",
        platelet: "Trombosit",
        granulocyte: "Granülosit",
        plasma: "Plazma",
        stemCell: "Kök Hücre",
        regardlessOfBloodType: "Kan Grubu Fark Etmeksizin",
        infoSiteTotal: "Bu site aracılığıyla bugüne kadar toplam",
        infoMainTitle: "{{count}} kan ilanı",
        infoHelpedPeople: "hazırlanarak {{count}} kişinin yaşama tutunmasına katkı sağlandı.",
        templateSelection: "Kan İlanı Şablon Seçenekleri",
      },
    },
    en: {
      translation: {
        topBarLeftButtonGroup: "Language, theme, and sound selection button group",
        topBarRightButtonGroup: "App counter information button group",
        regularNeedButtonGroup: "Regular need button group",
        regularNeedInfoButton: "Regular need information button",
        regularNeedInfoTitle: "What is Regular Need?",
        regularNeedInfoContent: "Regular Blood Need refers to the requirement for blood or blood components that is not a one-time occurrence but continues at certain intervals throughout the treatment process.\nFor example, patients undergoing oncology treatment, individuals in the organ transplantation process, or those receiving regular transfusions due to certain chronic illnesses fall under this category.\nPlease do not check this option for one-time or short-term needs.",
        topBarTrButton: "Turkish language selection button",
        topBarEnButton: "English language selection button",
        topBarThemeButton: "Theme toggle button",
        topBarSoundButton: "Sound reading button",
        topBarInfoButton: "App counter information button",
        greeting: "We wish you a speedy recovery. We hope suitable blood is found soon and you regain your health. We are with you!",
        close: "Close",
        headerTitle: "Blood Ad Creation Assistant",
        headerDesc: "After filling out the form, you can share the generated image with us and your surroundings by clicking the “Last Step” button.",
        fillAllFields: "<strong>Warning:</strong> Please complete <strong>{{missingFields}}</strong> {{noun}}.\nAccurate and complete information is essential for ensuring fast and correct communication during the blood donation process.",
        fillAllFieldsSingleNoun: "field",
        fillAllFieldsMultipleNoun: "fields",
        requiredText: "This field is required.",
        downloadButtonAlt: "Last Step: Save the Ad to Your Gallery",
        phoneNumberInvalid: "Please enter a valid phone number.\nThe number must be 11 digits, starting with “0”\n(e.g., 05XX XXX XX XX)",
        bloodGroup: "Blood Group",
        bloodGroupType: "Blood Group Type",
        bloodType: "Blood Type",
        donationType: "Donation Type",
        fullName: "Patient Name",
        phone: "Phone",
        date: "Date",
        regularNeedDate: "Regular Need",
        hospitalName: "Hospital Name",
        location: "Centers where blood donation can be made",
        select: "Select",
        submit: "Submit",
        legalNotice: "The personal data you enter is not recorded, stored, or shared with third parties in any physical or digital environment. This data is only displayed momentarily on the “Blood Announcement Form” and is completely removed after viewing.",
        redBlood: "Red Blood",
        platelet: "Platelet",
        granulocyte: "Granulocyte",
        plasma: "Plasma",
        stemCell: "Stem Cell",
        regardlessOfBloodType: "Regardless of Blood Type",
        infoSiteTotal: "Through this site, a total of",
        infoMainTitle: "{{count}} blood ads",
        infoHelpedPeople: "have been prepared, contributing to {{count}} people clinging to life.",
        templateSelection: "Blood Ad Template Options",
      },
    },
  },
  lng: "tr",
  fallbackLng: "tr",
  interpolation: { escapeValue: false },
});

export default i18n;